...
tf.app.flags.DEFINE_integer("mini_batch", 1024, "mini_batch")
tf.app.flags.DEFINE_string("outdata", 'D:\\edsr-data800.tfrecord', "outdata")
...
def createTFRecordForEDSR():
    ...
    with tf.Session() as sess:
        ...
        means=[]
        for i in range(FLAGS.mini_batch):
            croped_image=tf.random_crop(image, (FLAGS.image_size*FLAGS.scale, FLAGS.image_size*FLAGS.scale, 3))
            croped_image = tf.cast(croped_image, tf.float32)
            mean=tf.reduce_mean(croped_image)
            sub_image=tf.subtract(croped_image, mean)
        ...
        for i in range(len(file_list)):
            print('i',i)
            loaded_images,loaded_means=sess.run([images,means])
            
            for j in range(len(loaded_images)):                
                example = tf.train.Example(features=tf.train.Features(feature={
                    'label': tf.train.Feature(float_list=tf.train.FloatList(value=[loaded_means[j]])),
                    'image': tf.train.Feature(bytes_list=tf.train.BytesList(value=[loaded_images[j].tobytes()]))
                    }))
                writer.write(example.SerializeToString())
...
def dataset_input_fn():
    ...
    img = features['image']
    lr_images = tf.image.resize_images(img, (5, 5),method=tf.image.ResizeMethod.BICUBIC)
