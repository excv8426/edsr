...
def _get_weights(config):
  weights={
    'w1': tf.Variable(tf.random_normal([5, 5, config.c_dim, 128], stddev=np.sqrt(2.0/25/3)), name='w1'),
    'w2': tf.Variable(tf.random_normal([3, 3, 128, 64], stddev=np.sqrt(2.0/9/64)), name='w2'),
    'w3': tf.Variable(tf.random_normal([3, 3, 64, config.c_dim * config.scale * config.scale ], stddev=np.sqrt(2.0/9/32)), name='w3')
    }
  return weights
...
def SRResNet(config,normal_input):
  conv1=slim.conv2d(normal_input,config.feature_size,[3,3],padding='SAME')
  
  pre_res=conv1
  for i in range(config.num_layers):
    pre_res = resBlock(pre_res,config.feature_size,scale=0.1)
  conv6=pre_res
  
  conv7=slim.conv2d(conv6,config.feature_size,[3,3],padding='SAME')
  conv7=tf.add(conv1, conv7)
  conv7=slim.conv2d(conv7,config.feature_size,[3,3],activation_fn=None)
  
  conv8=slim.conv2d(conv7,config.c_dim * config.scale * config.scale,[3,3],padding='SAME',activation_fn=None)
  ps = PS(config,conv8, config.scale)
  
  #conv9=slim.conv2d(ps,3,[9,9],padding='SAME')
  return ps
